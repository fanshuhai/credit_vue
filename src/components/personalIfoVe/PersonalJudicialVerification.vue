			...
			<div class="newCheck_form">
				<el-form :inline="true" :model="newQuery" ref="newQuery" :rules="rules">
					<el-form-item label="姓名：" prop="name">
						<el-input v-model="newQuery.name" placeholder="请输入姓名"></el-input>
					</el-form-item>
					<el-form-item label="身份证号：" prop="idCard">
						<el-input v-model="newQuery.idCard" placeholder="身份证号"></el-input>
					</el-form-item>
					<el-form-item>
	            ...
	            this.$refs[formName].validate((valid)=>{ 
		            if(valid){
		                ...
		                this.$axios.get(this.HOST+'/api/v1/hfw/search',{
		                	params:{
			                    name: this.newQuery.name,
			                    cardId: this.newQuery.idCard,
		                	}
		                })
		                .then(res=>{
		                	console.log(res.data)
		                	if(res.data.judicial.success=='e'){
		                		this.$message.error(res.data.judicial.message);
		                	}
		                	let fxcontent=res.data.judicial.fxcontent;
		                	this.username=res.data.judicial.username;
		                	this.checkAllData='全部（'+res.data.judicial.fxmsgnum+'）';
		                	//遍历typeOption
